...
- **News** — add a `.qmd` file to `news/` with `title` and `date` in its
  front matter; the News page and the "Latest news" box on the home page
  (which shows the newest post) pick it up automatically.
...
- **Colors and fonts** — edit `_brand.yml`. Fonts are Google Fonts (Inter
  for text, Outfit for headings). The site has a single light theme.
- **Palette variables** — `styles-palette.scss` sets the navbar, footer,
  card and border colours. Its hex values mirror `_brand.yml`, so change
  both together.
